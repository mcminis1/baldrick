...
async def get_relevant_activities(user_question) -> str:
    completion = await openai.Completion.create(engine="text-davinci-003", prompt=EVENT_CHOOSER_PROMPT(user_question, event_names))
    return completion

async def get_sql_query(user_question, activities) -> str:
    completion = await openai.Completion.create(engine="text-davinci-003", prompt=QUERY_PROMPT(top_k, table_info, activities, activity_schema, user_question))
